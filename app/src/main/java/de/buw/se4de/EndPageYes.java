...
public class EndPageYes extends JFrame {
	...
	public EndPageYes() {
		...
		Home.setFont(new Font("Ebrima", Font.BOLD, 12));
		Home.setHorizontalAlignment(SwingConstants.CENTER);
		Home.setBackground(Color.WHITE);
		Home.setBounds(100, 50, 600, 30);
		Home.setOpaque(true);
		...
		double result = Home(amount, year);
		NumberFormat nf = NumberFormat.getNumberInstance(Locale.ENGLISH);
		DecimalFormat df = (DecimalFormat)nf;
//		DecimalFormat df = new DecimalFormat("####0.00");
		String compVal = df.format(result);
		Home.setText("<html>" + "With an inflation rate of "+ (expectedInflation*100) + 
				"%, your total amount (in EUR) after "+ c +" years will be: "+ "<br/>" + 
				 "----------------------------------------"
				 +compVal +" Euros" +
				 "----------------------------------------"+"</html>");
